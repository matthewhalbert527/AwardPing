-- Read-only smoke for the forward-only Stage 1 quarantine v3 accounting
-- evolution. No application or audit rows are created by this script.
...
begin
  ...
  then
    raise exception
      'The Stage 1 quarantine v3 catalog or SHA-256 prerequisite is incomplete.';
  end if;
  ...
  if v_definition_sha256 <>
    ...
    or pg_catalog.strpos(
      v_definition,
      $contract$array['boundary', 'cas', 'journal_phase', 'response_loss_possible']$contract$
    ) = 0
    or pg_catalog.strpos(
      v_definition,
      $contract$'journal_archive',
                'journal_persistence',$contract$
    ) = 0
    or pg_catalog.strpos(
      v_definition,
    ...
    or pg_catalog.strpos(
      v_definition,
      $contract$] in ('not_started', 'verified')$contract$
    ) = 0
    or pg_catalog.strpos(
      v_definition,
      $contract$'journal_archive', 'state'
            ] = 'not_started'$contract$
    ) = 0
    or pg_catalog.strpos(
      v_definition,
  ...
  then
    raise exception
      'The Stage 1 quarantine RPC does not have the exact v3 accounting-policy seal.';
  end if;
  ...
  v_constraint_sha256 := pg_catalog.encode(
    extensions.digest(
  ...
  then
    raise exception
      'The Stage 1 quarantine audit constraint does not preserve exactly paired v1/v2 history plus v3.';
  end if;
end;
$catalog_smoke$;
...
begin
  ...
  if not v_denied then
    raise exception 'The anon role invoked the Stage 1 quarantine v3 RPC.';
  end if;
end;
$anon_denied$;
reset role;
...
begin
  ...
  if not v_denied then
    raise exception
      'The authenticated role invoked the Stage 1 quarantine v3 RPC.';
  end if;
end;
