-- The reviewed commit RPC raised SQLSTATE 40001 for five fail-closed fences
-- whose expectations are baked into the call arguments, so an identical-args
-- retry can never succeed. PostgREST auto-retries 40001 as a serialization
-- failure, which turned every fence hit into a ~2-minute retry storm that
-- surfaced as 'upstream request timeout' and masked the real error (observed
-- live on the first reviewed Rhodes commit, 2026-08-20). 55000
-- (object_not_in_prerequisite_state) keeps the fail-closed semantics without
-- inviting automatic retries. This is the deployed definition with only the
-- five errcode values changed.
CREATE OR REPLACE FUNCTION public.commit_reviewed_stage1_reconciliation_publication(p_reconciliation_id uuid, p_shared_award_id uuid, p_expected_started_at timestamp with time zone, p_expected_queue_generation bigint, p_expected_award_updated_at timestamp with time zone, p_expected_public_facts jsonb, p_summary text, p_public_facts jsonb, p_confidence double precision, p_evidence_rows jsonb, p_source_ids uuid[], p_candidate_ids uuid[], p_generated_candidates jsonb, p_candidate_status_updates jsonb, p_audit_row jsonb, p_review_binding jsonb)
 RETURNS shared_award_reconciliation_queue
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
AS $function$
