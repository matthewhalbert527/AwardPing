-- list_stage1_effective_publication re-derives full visual/R2 evidence
-- snapshots (~20s) on every call once any cohort reaches verified_beta, which
-- took down every award page load after the first Rhodes promotion. The
-- computation is unchanged (moved verbatim to
-- private.stage1_effective_publication_compute); the public function now
-- serves a 60-second cache that also invalidates the moment any registry row
-- changes, so promotions and release transitions are visible immediately.
...
create table if not exists private.stage1_effective_publication_cache (
  id integer primary key check (id = 1),
  computed_at timestamptz not null,
  registry_high_water timestamptz not null,
  rows jsonb not null
);
revoke all on table private.stage1_effective_publication_cache
