-- Provably-inert expansion candidates (option C of
-- docs/stage1-inert-expansion-candidates.md, adopted 2026-08-30 under the
-- owner directive to complete all 25 cohorts). A candidate whose control
-- responded on every attempt while its bound content never became visible is
-- demonstrably not an expansion state of the page; coverage may then reach
-- verified_complete over the real states with the proof sealed per candidate
-- in the coverage metadata. Guardrails exactly as the memo required:
-- inertness is per-capture with per-candidate proof (attempts >= 2,
-- control_responded, content_never_visible), a page where any candidate
-- opened may not declare inert candidates (inert_count > 0 forces
-- retained_state_count = 0), and a non-responding control remains a failure.
...
begin
  ...
  -- option C, adopted 2026-08-30): optional for backward compatibility.
  v_inert_count := coalesce((v_coverage ->> 'inert_count')::numeric, 0);
  if v_inert_count < 0 then
    return false;
  end if;
  if v_inert_count > 0 then
    if v_retained_count <> 0
      or pg_catalog.jsonb_typeof(v_coverage -> 'inert_candidates') is distinct from 'array'
