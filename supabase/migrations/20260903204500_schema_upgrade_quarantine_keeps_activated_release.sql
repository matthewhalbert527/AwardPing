-- The dormant schema-upgrade quarantine path gets the activated-release guard
-- (owner directive 2026-09-02/03). quarantine_stage1_evidence_schema_upgrade_
-- failure cleared release_epoch on every verified cohort whose manifest pins
-- the failing source, relying on the source invalidation trigger to demote
-- afterwards. Under the activated release the trigger now only flags, which
-- would have left the cohort verified_beta with no epoch (cohort_release_
-- epoch_mismatch) - out of the live set. Released cohorts now keep their
-- epoch here too; pre-activation behaviour is unchanged. The flow is not in
-- the nightly arguments today; this closes it before it is ever run.
-- Generated by patching the live definition; only the two guards were added.

CREATE OR REPLACE FUNCTION public.quarantine_stage1_evidence_schema_upgrade_failure(p_source_id uuid, p_acquisition_id uuid, p_request_id uuid, p_reason_code text, p_evidence jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SET search_path TO ''
AS $function$
...
begin
  ...
  if private.stage1_evidence_schema_upgrade_has_exact_keys(
    ...
    or v_policy ->> 'policy_version' is distinct from '3'
  ...
  if v_initial_open then
    ...
    where registry.publication_state = 'verified_beta'
      ...
      -- 2026-09-03 (owner directive, publish now / refine daily): a cohort
      -- that is part of the ACTIVATED release keeps its epoch; the source
      -- invalidation trigger below records the failure as a re-verification
      -- flag while the last verified publication stays live.
      and not exists (
        select 1
