-- Give password recovery its own durable, account-independent IP budget. Keep
-- the existing advisory-lock reservation semantics so parallel serverless
-- requests cannot oversubscribe the limit.
...
begin
  if p_kind not in ('subscribe', 'contact', 'source_request', 'password_recovery') then
...
comment on function public.reserve_public_form_rate_limit(text, text, integer, integer) is
  'Atomically reserves one public subscribe/contact/source-request/password-recovery attempt under an exact kind/IP rolling window.';
