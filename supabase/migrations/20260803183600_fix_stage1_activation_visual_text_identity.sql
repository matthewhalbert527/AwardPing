-- The human-reviewed retained capture and the browser-rendered visual capture
-- are separate whole-page representations. Bind each one independently:
-- the top-level observed hash proves the reviewed retained capture, while the
-- local normalized hash identifies the persisted visual baseline whose raw
-- semantic text and image hashes are mirrored to R2.
...
comment on function private.stage1_activation_persistence_evidence_valid(
  ...
  'Validates v3 Stage 1 proof with distinct reviewed-retained and browser-visual whole-page text identities plus raw local/R2 artifact integrity.';
