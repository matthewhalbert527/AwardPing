-- Content addressing (repo commit 1adcd92) made a capture generation's
-- captured_at DURABLE: an unchanged re-capture of a stable page reuses the
-- generation and never advances the pointer timestamp. The publication fences
-- still required snapshot.latest_captured_at to be under 24 hours old, so any
-- cohort whose pages simply had not changed for a day could never promote,
-- while recency was already (and correctly) enforced via source.last_checked_at,
-- which every observation refreshes. This migration re-declares the two
-- functions with only that one predicate removed; the evidence-identity
-- comparisons on captured_at (is distinct from) remain.
