...
export const STAGE1_EVIDENCE_SCHEMA_UPGRADE_QUARANTINE_V3_ACCOUNTING_MIGRATION =
  "20260815083322_stage1_evidence_schema_upgrade_quarantine_v3_accounting.sql";
export const STAGE1_EVIDENCE_SCHEMA_UPGRADE_QUARANTINE_V3_ACCOUNTING_SMOKE =
  "stage1_evidence_schema_upgrade_quarantine_v3_accounting_smoke.sql";
...
export function listStage1EvidenceSchemaUpgradeQuarantineV3PriorVersions() {
  const targetVersion =
  ...
  if (versions.length === 0) {
    throw new Error("The Stage 1 quarantine v3 migration has no prior migration preflight set.");
  }
...
export function renderStage1EvidenceSchemaUpgradeQuarantineV3RollbackProbe() {
  const template = readFileSync(
    resolve(
      scriptDirectory,
      "sql/stage1-evidence-schema-upgrade-quarantine-v3-accounting-rollback-probe.sql",
    ),
    "utf8",
  );
