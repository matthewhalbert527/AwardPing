...
import { monitoringPromotionMatcherBundleHash } from "./monitoring-promotion-matcher-bundle.mjs";
import {
...
export const changeEventSuppressionMatcherBundleHash =
  monitoringPromotionMatcherBundleHash;
export const changeEventSuppressionMatcherSourceHash =
  changeEventSuppressionMatcherBundleHash;
...
const conditionalSourceShapePattern =
  ...
  /\b(?:profile|profiles|recipients?|awardees?|testimonial|news|press release|events? calendar|information sessions?|webinars?)\b|\/(?:profile|profiles|recipients?|awardees?|news|press|events?|calendar|register|registration|information-sessions?|admission-events|webinars?)(?:[/?#]|\s|$)/i;
...
export function isChangeEventSuppressed(change) {
...
export function changeEventSuppressionDecision(change, source = null, options = {}) {
  ...
  if (!retroSweep && source?.admin_review_status && source.admin_review_status !== "open") {
...
export function changeEventMatchesMonitoringPolicyRule(
  change,
  source = null,
  ruleId,
) {
  if (!change) return false;
  const targetRuleId = reviewableMonitoringPolicyFlagIdForAlias(ruleId);
  ...
  if (
    changeEventQualityFlags(details).some(
      (flag) => reviewableMonitoringPolicyFlagIdForAlias(flag) === targetRuleId,
    )
  ) {
    return true;
  }

  return textDetectedPolicyFlags({
    details,
    structured,
    summaryText,
    hasApplicantSignal: applicantSignalPattern.test(summaryText),
    hasDeterministicApplicantSignal:
      applicantSignalPattern.test(deterministicEvidenceText),
    sourceText,
  }).includes(targetRuleId);
}

function changeEventQualityFlags(details) {
  const structured = objectValue(details.structured_diff);
  return [
...
function textDetectedPolicyFlags({
  ...
  if (hasRelativeAgeOnlyPolicyChange({
    ...
    before: details.exact_before || details.before,
    after: details.exact_after || details.after,
    addedText: structured.added_text,
    removedText: structured.removed_text,
    dateChanges: structured.date_changes,
    amountChanges: structured.amount_changes,
...
function hasSupportedCorrectedEvidence(details) {
