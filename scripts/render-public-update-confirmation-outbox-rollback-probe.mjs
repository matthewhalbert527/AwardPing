...
export const PUBLIC_UPDATE_CONFIRMATION_OUTBOX_MIGRATION =
  "20260815023357_durable_public_update_confirmation_outbox.sql";
export const PUBLIC_UPDATE_CONFIRMATION_OUTBOX_SMOKE =
  "public_update_confirmation_outbox_smoke.sql";
...
export function listPublicUpdateConfirmationOutboxPriorMigrationVersions() {
  const targetVersion = PUBLIC_UPDATE_CONFIRMATION_OUTBOX_MIGRATION.split(
    "_",
    1,
  )[0];
  ...
  if (versions.length === 0) {
    throw new Error("The confirmation outbox patch has no prior migration preflight set.");
  }
...
export function renderPublicUpdateConfirmationOutboxRollbackProbe() {
  const template = readFileSync(
    resolve(
      scriptDirectory,
      "sql/public-update-confirmation-outbox-rollback-probe.sql",
    ),
    "utf8",
  );
  ...
  const migration = exactSqlBlock({
    ...
    sql: readFileSync(
      resolve(migrationsDirectory, PUBLIC_UPDATE_CONFIRMATION_OUTBOX_MIGRATION),
      "utf8",
    ),
  });
...
if (invokedPath === import.meta.url) {
  process.stdout.write(renderPublicUpdateConfirmationOutboxRollbackProbe());
}
