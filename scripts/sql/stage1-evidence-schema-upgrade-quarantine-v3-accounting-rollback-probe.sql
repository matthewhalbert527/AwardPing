...
--   node scripts/run-stage1-evidence-schema-upgrade-quarantine-v3-accounting-rollback-probe.mjs
-- The generated SQL applies exactly the linked migration and read-only smoke
-- inside a transaction, rolls back, and proves the complete prior contract.

begin;

create temporary table awardping_stage1_quarantine_v3_probe_baseline (
...
create function pg_temp.awardping_stage1_quarantine_v3_probe_assert(
  p_condition boolean,
  p_message text
)
returns void
...
begin
  if coalesce(p_condition, false) is not true then
    raise exception using
      errcode = 'P0001',
      message = 'Stage 1 quarantine v3 rollback probe failed: ' || p_message;
  end if;
end;
$$;

create function pg_temp.awardping_stage1_quarantine_v3_function_contract(
...
create function pg_temp.awardping_stage1_quarantine_v3_application_contract()
returns jsonb
language sql
stable
set search_path = ''
...
-- __AWARDPING_EXACT_PRIOR_MIGRATION_VERSIONS__
  ...
  v_target_version constant text := '20260815083322';
...
begin
  ...
  perform pg_temp.awardping_stage1_quarantine_v3_probe_assert(
  ...
  v_function_contract :=
    pg_temp.awardping_stage1_quarantine_v3_function_contract(true);
  v_function_metadata_contract :=
    pg_temp.awardping_stage1_quarantine_v3_function_contract(false);
  perform pg_temp.awardping_stage1_quarantine_v3_probe_assert(
  ...
  insert into pg_temp.awardping_stage1_quarantine_v3_probe_baseline (
    ...
    pg_temp.awardping_stage1_quarantine_v3_application_contract()
  );
end;
$preflight$;
...
begin
  perform pg_temp.awardping_stage1_quarantine_v3_probe_assert(
    pg_temp.awardping_stage1_quarantine_v3_function_contract(false) = (
      ...
      from pg_temp.awardping_stage1_quarantine_v3_probe_baseline baseline
    ),
  ...
  perform pg_temp.awardping_stage1_quarantine_v3_probe_assert(
    ...
    'the exact quarantine RPC definition did not change in the migration transaction'
  );
  perform pg_temp.awardping_stage1_quarantine_v3_probe_assert(
    pg_catalog.encode(
    ...
    'the applied quarantine RPC definition does not have the reviewed v3 digest'
  );
  ...
  perform pg_temp.awardping_stage1_quarantine_v3_probe_assert(
    pg_catalog.encode(
      extensions.digest(
        pg_catalog.convert_to(v_constraint_definition, 'UTF8'),
        'sha256'
      ),
      'hex'
    ) = '82bfc427d568cb7ddedcd56d9ef8fa16dd61c769395f9a30e5466c0e387160c6',
    'the applied quarantine audit constraint does not have the reviewed v3 digest'
  );
  perform pg_temp.awardping_stage1_quarantine_v3_probe_assert(
    pg_temp.awardping_stage1_quarantine_v3_application_contract() = (
  ...
  perform pg_temp.awardping_stage1_quarantine_v3_probe_assert(
    not exists (
      select 1
      from supabase_migrations.schema_migrations migration
      where migration.version = '20260815083322'
    ),
