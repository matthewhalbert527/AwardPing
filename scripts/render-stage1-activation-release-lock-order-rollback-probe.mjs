...
export const STAGE1_ACTIVATION_RELEASE_LOCK_ORDER_MIGRATION =
  "20260814223000_order_stage1_activation_release_locks.sql";
export const STAGE1_ACTIVATION_RELEASE_LOCK_ORDER_SMOKE =
...
export function listStage1ActivationReleaseLockOrderPriorMigrationVersions() {
  const targetVersion = STAGE1_ACTIVATION_RELEASE_LOCK_ORDER_MIGRATION.split(
    "_",
    1,
  )[0];
  ...
  if (versions.length === 0) {
    throw new Error("The Stage 1 release-lock patch has no prior migration preflight set.");
  }
...
export function renderStage1ActivationReleaseLockOrderRollbackProbe() {
  const template = readFileSync(
    resolve(
      scriptDirectory,
      "sql/stage1-activation-release-lock-order-rollback-probe.sql",
    ),
    "utf8",
  );
  for (const marker of [
...
if (invokedPath === import.meta.url) {
  process.stdout.write(renderStage1ActivationReleaseLockOrderRollbackProbe());
}
