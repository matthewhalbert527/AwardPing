...
function foldedTokenBag(element: string) {
...
function listGroupSubsumed(smallElements: string[], largeElements: string[]) {
...
function mergeSubsumedListGroups(groups: Map<string, FactSelection[]>) {
  ...
  for (const small of entries) {
    ...
    if (host) {
      groups.set(host.key, [...(groups.get(host.key) as FactSelection[]), ...(groups.get(small.key) as FactSelection[])]);
...
const conflictOrdinalRanks: Record<string, number> = { first: 1, "1st": 1, second: 2, "2nd": 2, third: 3, "3rd": 3, fourth: 4, "4th": 4, last: -1 };

function conflictDateKey(normalizedValue: string): string | null {
  // The value is already lowercased with every non-alphanumeric run collapsed
  // to single spaces, so plain space-delimited patterns are exact here.
...
function dateSatisfiesNthRule(dateKey: string, nthKey: string): boolean {
...
function mergeCompatibleDateGroups(groups: Map<string, FactSelection[]>) {
  const dateKeys = () => [...groups.keys()].filter((key) => key.startsWith("date:"));
